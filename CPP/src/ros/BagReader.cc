...
void BagReader::SaveData(std::string folderPath,std::string dataType){
  ...
  BOOST_FOREACH(rosbag::MessageInstance m,view) {
    if (m.getDataType() == "sensor_msgs/Image" &&
      ...
      cv_image_ptr = cv_bridge::toCvCopy(imgMsg, sensor_msgs::image_encodings::BGR8);
      ...
      ss << "/" << std::fixed << std::setprecision(6) << imgMsg->header.stamp.toSec()
         << ".jpg";
      ...
      sensor_msgs::CompressedImage::ConstPtr imgMsg = m.instantiate<sensor_msgs::CompressedImage>();
      cv_image_ptr = cv_bridge::toCvCopy(imgMsg, sensor_msgs::image_encodings::BGR8);
      cv_image = cv_image_ptr->image;
      ss<<folderPath<<topicMap_[m.getTopic()];
      ...
      sensor_msgs::PointCloud2::ConstPtr pcdMsg = m.instantiate<sensor_msgs::PointCloud2>();
      std::stringstream ss;
      ss<<folderPath<<topicMap_[m.getTopic()];
      if(access(ss.str().c_str(),0)==-1){
        mkdir(ss.str().c_str(),0777);
      }
      ss<<"/"<<std::fixed<<std::setprecision(6)<<pcdMsg->header.stamp.toSec()<<".pcd";
...
void BagReader::SaveData(std::string folderPath,std::string dataType,std::string dataTopic){
  rosbag::View view(*bag_);
  std::vector<int> compression_params;
  compression_params.push_back(CV_IMWRITE_JPEG_QUALITY);  //选择jpeg
  compression_params.push_back(100);  //在这个填入你要的图片质量
  ...
  BOOST_FOREACH(rosbag::MessageInstance m,view) {
    if (m.getDataType() == "sensor_msgs/Image" &&
      ...
      sensor_msgs::PointCloud2::ConstPtr pcdMsg = m.instantiate<sensor_msgs::PointCloud2>();
      std::stringstream ss;
      ss <<folderPath<< std::fixed << std::setprecision(6) << m.getTime().toSec() << ".pcd";
      pcl::PointCloud<pcl::PointXYZI>cloud;
      pcl::fromROSMsg(*pcdMsg,cloud);
      std::cout<<"Save file: "<<ss.str()<<std::endl;
      pcl::io::savePCDFile(ss.str(),cloud);
...
void BagReader::SaveData(std::string folderPath,std::string dataType,std::string dataTopic,std::vector<double> timeList){
  rosbag::View view(*bag_);
  std::vector<int> compression_params;
  int timeIdx=0;
  ...
  BOOST_FOREACH(rosbag::MessageInstance m,view) {
    if (m.getDataType() == "sensor_msgs/Image" &&
      ...
      if(saveOrNotSave){
        ...
        ss << folderPath<< std::fixed << std::setprecision(6) << imgMsg->header.stamp.toSec() << ".jpg";
        std::cout<<"Save file: "<<ss.str()<<std::endl;
        cv::imwrite(ss.str(),cv_image,compression_params);
      }
      ...
      if(saveOrNotSave){
        std::stringstream ss;
        ss << folderPath<<std::fixed << std::setprecision(6) << pcdMsg->header.stamp.toSec() << ".pcd";
        pcl::PointCloud<pcl::PointXYZI>cloud;
        pcl::fromROSMsg(*pcdMsg,cloud);
        std::cout<<"Save file: "<<ss.str()<<std::endl;
        pcl::io::savePCDFile(ss.str(),cloud);
